    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          @click="toggleHighlight(author)"
          :class="{
            highlight: highlightedAuthor?.id === author.id,
            'text-bold': highlightedAuthor?.id === author.id,
          }"
          :style="{
            color: highlightedAuthor?.id === author.id ? 'white' : '#333',
            cursor: 'pointer',
          }"
        >
          {{ author.name }} ({{ author.birthYear }})
...
const toggleHighlight = (author) => {
  highlightedAuthor.value = highlightedAuthor.value?.id === author.id ? null : author
}
</script>
